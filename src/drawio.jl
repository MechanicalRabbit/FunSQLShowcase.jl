...
const DRAWIO_CONFIG =
    (
        ...
        css =
            """
            @import url("https://cdn.jsdelivr.net/gh/fonsp/Pluto.jl@0.20.13/frontend/fonts/alegreya.css");
            @import url("https://cdn.jsdelivr.net/gh/fonsp/Pluto.jl@0.20.13/frontend/fonts/vollkorn.css");
            @import url("https://cdn.jsdelivr.net/gh/fonsp/Pluto.jl@0.20.13/frontend/fonts/juliamono.css");
            """,
    )

"""
    DrawIO(filename; editable = true, data = nothing)

Embed a [DrawIO](https://drawio.com) diagram in the notebook.

This widget displays an SVG image generated by DrawIO, a web-based diagram editor.  Clicking the image
opens an embedded DrawIO editor, allowing you to edit the diagram directly within the notebook.

The diagram is stored as an SVG file on the local filesystem, so it must be distributed together with
the notebook.  However, when the notebook is exported as static HTML, the SVG is inlined in the HTML
output and the external file is no longer required.

You can include placeholders of the form `\$(identifier)` in any text label within the diagram.  These
placeholders are replaced with values provided via `data` parameter.

- `filename`: Path to the SVG file storing the diagram.  If the file does not exist, a "Click to edit"
  image is shown.  Changes made in the embedded DrawIO editor are saved back to this file.
- `editable`: If `true`, clicking the image opens an embedded DrawIO editor.  In static HTML exports,
  editing is always disabled regardless of this setting.
- `data`: A dictionary, named tuple, or one-row table used to substitute identifiers referenced in text
  labels.
"""
struct DrawIO
    filename::String
    ...
    show(
        ...
        <figure>
          <script>
            ...
            const figureNode = currentScript.closest("figure")
            const svgNode = figureNode.querySelector("svg")
            console.assert(svgNode, "%s is not a valid SVG image", $(drawio.filename))
